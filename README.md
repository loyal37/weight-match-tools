...
面板顶部自带 **Language / 语言** 切换（默认**中文**，可选 English），不依赖 Blender 的全局语言设置；切换即时生效，映射表和设置会原样保留。
...
- **Lock Same Names**：两边同名的组直接锁定配对，不参与自动分配。⚠️ 若两边都用数字命名但编号体系不同（如 A 的 `3` 和 B 的 `3` 不是同一根骨骼），请关闭此项。
- **Match All (Force)**：强制全量匹配——高质量配对保持 1:1 不变，其余源组（包括没有权重的空骨骼组）按相似度从高到低认领剩余目标组；配合 *Allow Many-to-One* 时，弱权重骨骼会**合并进它最相似的目标组**（而不是抢一个无关空位），超额的并进最接近的已用组。配合 *Fill Missing Groups*，可让源物体处理后**恰好拥有目标物体的全部组名**（例如 0~360 全数字命名）。
- **Match Target Order**：Apply 之后把源物体的顶点组列表按目标物体的顺序重建（权重按名字跟随，骨架绑定不受影响），例如最终就是 0,1,2…360 的顺序。
...
1. 勾选 **Match All (Force)** 和 **Fill Missing Groups**；
2. **Auto Match Groups** → 非空组按权重场精确配对（顶部骨骼通常 0.9+），空组排最后认领剩余数字名；
3. **Apply to Source (Rename/Merge)** → 源物体组名集合与目标完全一致，且真实权重不会互相污染（非空组各自独占数字名，合并的只有空组）。
